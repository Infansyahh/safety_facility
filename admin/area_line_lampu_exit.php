...
if (isset($_POST['tambah_line'])) {
    ...
    header("Location: area_line_lampu_exit.php?msg=tambah");
    exit();
}
...
if (isset($_POST['edit_line'])) {
    ...
    $nama = trim($_POST['nama_line']);
    $stmt = mysqli_prepare($koneksi, "UPDATE area_line SET nama_line = ? WHERE id_line = ? AND jenis = 'lampu_exit'");
    mysqli_stmt_bind_param($stmt, "si", $nama, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location: area_line_lampu_exit.php?msg=edit");
    exit();
}
...
if (isset($_GET['hapus'])) {
    ...
    $cek = mysqli_query(
        ...
        "SELECT COUNT(*) as total FROM `master_lampu`
         WHERE `code` LIKE 'LE%' AND `line_area` = (SELECT nama_line FROM area_line WHERE id_line = $id)"
    );
    $row_cek = mysqli_fetch_assoc($cek);
    if ($row_cek['total'] > 0) {
        header("Location: area_line_lampu_exit.php?msg=gagal_hapus");
    } else {
        $stmt = mysqli_prepare($koneksi, "DELETE FROM area_line WHERE id_line = ? AND jenis = 'lampu_exit'");
        mysqli_stmt_bind_param($stmt, "i", $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        header("Location: area_line_lampu_exit.php?msg=hapus");
    }
    exit();
}
...
if ($cari !== '') {
    ...
    $stmt = mysqli_prepare(
        ...
        "SELECT a.*, COUNT(m.id) as total_item
         FROM area_line a
         LEFT JOIN `master_lampu` m ON m.`line_area` = a.nama_line AND m.code LIKE 'LE%'
         WHERE a.jenis = 'lampu_exit' AND a.nama_line LIKE ?
...
<head>
    ...
    <title>Area Line Lampu Exit</title>
    ...
    <style>
        ...
        table.area-table tbody tr:hover td {
            background: #f8fafc;
        }
...
<body>
        ...
        <ul class="sidebar-menu">
            ...
            <li class="has-submenu active">
                ...
                <ul class="submenu" style="display:block;">
                    <li class=""><a href="area_line_lampu_emergency.php">Lampu Emergency</a></li>
                    <li class="active"><a href="area_line_lampu_exit.php">Lampu Exit</a></li>
        ...
        <section class="content-body">
            ...
            <?php if (isset($_GET['msg'])): ?>
                ...
                <?php else: ?>
                    <div class="alert-success"><i class="fa-solid fa-circle-check"></i>
                        <?php $p = ['tambah' => 'Area Line baru berhasil ditambahkan.', 'edit' => 'Area Line berhasil diperbarui.', 'hapus' => 'Area Line berhasil dihapus.'];
                        echo $p[$_GET['msg']] ?? 'Berhasil.'; ?>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
            ...
            <div class="page-toolbar">
                <form method="GET" class="search-bar">
                    ...
                    <?php if ($cari !== ''): ?>
                        <a href="area_line_lampu_exit.php" class="btn btn-outline"><i class="fa-solid fa-rotate-left"></i> Reset</a>
                    <?php endif; ?>
                </form>
                            ...
                            while ($row = mysqli_fetch_assoc($result)): ?>
                                <tr>
                                    ...
                                    <td class="action-icons">
                                        ...
                                        <a href="area_line_lampu_exit.php?hapus=<?= $row['id_line']; ?>" class="ic-delete" title="Hapus"
                        ...
                        <?php else: ?>
                            <tr class="empty-row">
                                <td colspan="5">
                                    ...
                                    Belum ada area line untuk Lampu Exit.
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    <div class="modal-overlay" id="modalTambah">
        <div class="modal-box">
            <h3><i class="fa-solid fa-door-open"></i> Tambah Area Line â€” Lampu Exit</h3>
            <form method="POST">
                <div class="form-group">
                    ...
                    <p class="input-hint">Area ini khusus untuk Lampu Exit.</p>
                </div>
        ...
        <div class="modal-box">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <p class="input-hint">Perubahan nama akan otomatis berlaku di data master Lampu Exit.</p>
                </div>
                <div class="modal-footer">
