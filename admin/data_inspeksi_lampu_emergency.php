...
// Menggunakan penanganan konversi bulan Indonesia ke angka/Inggris untuk kecocokan query MySQL
$list_bulan = [
    'January' => 1,
    'February' => 2,
    'March' => 3,
...
$bulan_param = isset($_GET['bulan']) ? $_GET['bulan'] : date('F');
$tahun       = isset($_GET['tahun']) ? (int)$_GET['tahun'] : (int)date('Y');
$cari        = isset($_GET['cari'])  ? trim($_GET['cari']) : '';

$angka_bulan = $list_bulan[$bulan_param] ?? (int)date('n');

/* ---- HAPUS ---- */
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    $id   = (int)$_GET['hapus'];
    $stmt = mysqli_prepare($koneksi, "DELETE FROM `inspeksi_lampu` WHERE `id_inspeksi` = ?");
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location: data_inspeksi_lampu_emergency.php?bulan=" . urlencode($bulan_param) . "&tahun=$tahun&msg=hapus");
    exit();
}
...
$bulan_indo = $db[$bulan_param] ?? $bulan_param;
...
<head>
    ...
    <title>Laporan Lampu Emergency - <?= htmlspecialchars($bulan_indo) . ' ' . $tahun; ?></title>
    ...
    <main class="main-content">
        <header class="topbar">
            ...
            <div class="topbar-right">
                <div class="user-profile">
                    <span>Hi, <strong><?= htmlspecialchars($_SESSION['nama_lengkap'] ?? ($_SESSION['nama_operator_popup'] ?? 'Admin')); ?></strong></span>
        ...
        <section class="content-body">
            ...
            <div class="page-head">
                ...
                <div class="ph-info">
                    <h2>Laporan Inspeksi Lampu Emergency</h2>
                    <p><?= htmlspecialchars($bulan_indo) . " " . $tahun; ?></p>
                </div>
            </div>

            <div class="stats-row">
                <div class="s-card">
                    <div class="s-icon" style="background:#f59e0b"><i class="fa-solid fa-list-check"></i></div>
                    <div class="s-info">
                        <h4>Total Data</h4>
                        <p><?= $total; ?></p>
                    </div>
            ...
            <div class="action-bar">
                <div class="ab-left">
                    <a href="laporan_lampu_emergency.php?tahun=<?= $tahun; ?>" class="btn btn-back">
                        <i class="fa-solid fa-arrow-left"></i> Kembali
                    </a>
                    <a href="data_inspeksi_lampu_emergency.php?bulan=<?= urlencode($bulan_param); ?>&tahun=<?= $tahun; ?>&cari=<?= urlencode($cari); ?>&export=excel"
                        class="btn btn-excel">
                ...
                <div class="ab-right">
                    <form method="GET" class="search-wrap">
                        <input type="hidden" name="bulan" value="<?= htmlspecialchars($bulan_param); ?>">
                        <input type="hidden" name="tahun" value="<?= $tahun; ?>">
                        <input type="text" name="cari" placeholder="Cari nama, Code, lokasi..." value="<?= htmlspecialchars($cari); ?>">
                        <button type="submit" class="btn btn-search"><i class="fa-solid fa-magnifying-glass"></i></button>
                        <?php if ($cari !== ''): ?>
                            <a href="data_inspeksi_lampu_emergency.php?bulan=<?= urlencode($bulan_param); ?>&tahun=<?= $tahun; ?>"
                        ...
                        <?php if ($total > 0): ?>
                            ...
                            foreach ($rows as $r): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="data_inspeksi_lampu_emergency.php?bulan=<?= urlencode($bulan_param); ?>&tahun=<?= $tahun; ?>&cari=<?= urlencode($cari); ?>&hapus=<?= $r['id_inspeksi']; ?>"
