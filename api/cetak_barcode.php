<?php
/**
 * api/cetak_barcode.php - Preview barcode tunggal (PNG)
 * Param: id, type = lampu | p3k | eyewash
 */
if (ob_get_length()) {
    ob_end_clean();
}
...
if ($type === 'p3k') {
    ...
    if (!$data) {
        die('Data Kotak P3K tidak ditemukan.');
    }
} elseif ($type === 'eyewash') {
    $query = mysqli_query($koneksi, "SELECT * FROM master_eyewash WHERE id = $id");
    $data = mysqli_fetch_assoc($query);
    if (!$data) {
        die('Data Eye Wash tidak ditemukan.');
    }
} else {
    $type = 'lampu';
    ...
    $data = mysqli_fetch_assoc($query);
    if (!$data) {
        die('Data lampu tidak ditemukan.');
    }
}
...
if ($type === 'p3k') {
    ...
    $templatePath = __DIR__ . '/../foto/template_barcode_exit.png';
...
$qrContent = $data['code'];
$qrFile = $tempDir . 'qr_' . $type . '_' . $id . '_' . uniqid() . '.png';
